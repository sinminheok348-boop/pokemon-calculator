...
// 추천 포켓몬 결과 저장용
let recommendationWithMega = [];
let recommendationWithoutMega = [];
let currentRecommendationMode = "withMega";
...
window.switchRecommendation = function(mode) {
  currentRecommendationMode = mode;

  if (mode === "withMega") {
    renderRecommendationTable(recommendationWithMega);
  } else {
    renderRecommendationTable(recommendationWithoutMega);
  }
};
...
// 🟢 메가 포함
recommendationWithMega = recommendationList;

// 🟢 메가 제외
recommendationWithoutMega = recommendationList.filter(p => p.form !== "메가");

// 최초 표
renderRecommendationTable(recommendationWithMega);
currentRecommendationMode = "withMega";
};
