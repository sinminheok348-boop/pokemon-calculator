...
// 🔹 메가 포함 / 메가 제외 분리
const recommendationWithMega = recommendationList;
const recommendationNoMega = recommendationList.filter(p =>
  p.pokemonObj?.Form !== "메가" &&
  p.pokemonObj?.Form !== "원시" &&
  !p.pokemonObj?.Pokemon_Name.includes("원시")
);
...
document.addEventListener("click", (e) => {
  ...
  document.getElementById("recommendationTable-withMega").classList.toggle(
    "hidden", tab !== "withMega"
  );
  document.getElementById("recommendationTable-noMega").classList.toggle(
    "hidden", tab !== "noMega"
  );
});